# 使用 CIFAR-10 数据集的 MoE Tutorial 实验

一个面向入门与实践的 MoE（Mixture of Experts）小项目。记录了围绕 **CIFAR-10 图像分类** 做的几组实验：从普通 CNN baseline 出发，逐步过渡到带有路由机制的 MoE 结构，并继续把专家形式从“hidden feature mixing”推进到更接近原生 FFN-MoE 的实现。
...
在前一个版本取得的稳健参数基础上，进一步把专家改写成更接近原生MoE中的形态：
- 每个专家不再只输出 hidden，而是直接完成一套更完整的前馈映射；
- 在输出层由 gate 对多个专家输出进行 Top-k 加权融合。
