...
def show_samples(dataset, class_names, num_images=8, save_path=None):
    ...
    plt.suptitle("CIFAR-10 MoE Samples")
    plt.tight_layout()
...
# =========================
# 6. 可切换“均匀路由 / top-k 路由”的 MoE 头
# =========================
class WarmupTopKHiddenMixMoEHead(nn.Module):
    def __init__(
        self,
        input_dim,
        hidden_dim,
    ...
    def forward(self, x, mode="uniform"):
        ...
        mixed_hidden = (route_probs_expanded * expert_hiddens).sum(dim=1)
        logits = self.shared_classifier(mixed_hidden)

        return logits, route_probs, mixed_hidden, full_gate_probs
...
class CIFAR10MoECNN(nn.Module):
    def __init__(
        ...
        self.moe_head = WarmupTopKHiddenMixMoEHead(
            input_dim=128 * 8 * 8,
    ...
    def forward(self, x, mode="uniform"):
        x = self.features(x)
        x = self.flatten(x)
        logits, gate_probs, mixed_hidden, full_gate_probs = self.moe_head(x, mode=mode)
...
for epoch in range(num_epochs):
    ...
    if test_acc > best_test_acc:
        ...
        best_model_path = os.path.join(OUTPUT_DIR, "cifar10_moe_best_model.pth")
...
# 13. 画训练曲线
# =========================
loss_curve_path = os.path.join(OUTPUT_DIR, "cifar10_moe_loss_curve.png")
acc_curve_path = os.path.join(OUTPUT_DIR, "cifar10_moe_accuracy_curve.png")

plt.figure(figsize=(8, 5))
plt.plot(train_losses, label="train total loss")
plt.plot(test_losses, label="test loss")
plt.title("CIFAR-10 MoE - Loss")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.legend()
plt.grid(True)
...
plt.title("CIFAR-10 MoE - Accuracy")
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.legend()
plt.grid(True)
...
def show_predictions(model, dataset, class_names, device, num_images=8, save_path=None):
    if save_path is None:
        save_path = os.path.join(OUTPUT_DIR, "cifar10_moe_predictions.png")
    ...
    plt.suptitle("CIFAR-10 MoE Predictions")
    plt.tight_layout()
    plt.savefig(save_path, dpi=200, bbox_inches="tight")
    plt.show()
    plt.close()
...
def analyze_class_expert_preference(model, dataloader, class_names, device, num_experts, save_path=None):
    if save_path is None:
        save_path = os.path.join(OUTPUT_DIR, "cifar10_moe_class_expert_preference.txt")
...
# 16. 保存最终模型与训练记录
# =========================
final_model_path = os.path.join(OUTPUT_DIR, "cifar10_moe_final_model.pth")
torch.save(model.state_dict(), final_model_path)
print(f"saved final model to: {final_model_path}")

metrics_path = os.path.join(OUTPUT_DIR, "cifar10_moe_training_metrics.txt")
